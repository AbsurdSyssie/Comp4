...
class createaccount(QtGui.QWidget):
    ...
    def checktype(self):
        ...
        forms = True
        try:
            print(type(self.surnameBox.text()))
            int(self.surnameBox.text())
            
            valid = False
            self.cannotusenumbers()
        except:
            print("not a number")
        ...
        try:
            int(self.formBox.text())
            int(self.surnameBox.text())
            int(self.forenameBox.text())
            forms = False
            self.cannotusenumbers()
            
        except:
            if not self.forenameBox.text().isalpha():
                forms = False
                text = "Forname Box"
                self.cannotusenumbers(text)
                pass
            if not self.surnameBox.text().isalpha():
                forms = False
                text = "Surname Box"
                self.cannotusenumbers(text)
                pass
            if not self.middlenameBox.text().isalpha():
                forms = False
                text = "Middlename Box"
                self.cannotusenumbers(text)            
    ...
    def cannotusenumbers(self, text):
        reply = QtGui.QMessageBox()
        reply.setText("You cannot enter numbers in the " +text+ "field.")
        reply.setIcon(3)
        reply.addButton(QtGui.QPushButton('OK'), QtGui.QMessageBox.YesRole)
